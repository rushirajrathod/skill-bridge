...
async def generate_user_roadmap(request: RoadmapRequest):
    ...
    if request.target_job_description and request.target_job_description.strip():
        ...
        missing_skills = extract_skills_from_jd(
    ...
    if request.persona == "mentor" and request.current_role:
        mentorship_guide = generate_mentorship_guide(request.current_role, request.target_role, missing_skills)
    ...
    # 3. Compute Analytics
    match_pct = calculate_ml_match_percentage(current_skills, request.target_role, missing_skills)
